...
export const en = {
  ...
  reasoningDisabled: 'No reasoning',
  reasoningLevels: 'Custom levels',
  reasoningHint: 'Each level maps to the value sent to the gateway; an empty off sends no thinking parameter.',
  reasoningLevelToggle: 'Offer level',
  reasoningWireLabel: 'Wire value',
  modelReasoningEmpty: 'Reasoning levels are empty; enable at least one level, or set the model to not reason.',
  modelReasoningNoLevel: 'Reasoning levels only declare off; add a thinking level (e.g. high), or disable reasoning.',
  modelReasoningWireMissing: 'Every enabled thinking level needs the value sent to the gateway; only off may be empty.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  reasoningDisabled: '不支持思考',
  reasoningLevels: '自定义档位',
  reasoningHint: '每个档位对应发送给网关的值；off 留空表示不发送思考参数。',
  reasoningLevelToggle: '启用档位',
  reasoningWireLabel: '发送值',
  modelReasoningEmpty: '推理档位为空；请至少启用一个档位，或将推理等级设为不支持思考。',
  modelReasoningNoLevel: '推理档位只声明了 off；请添加思考档位（如 high），或关闭思考。',
  modelReasoningWireMissing: '每个启用的思考档位都需要填写发送给网关的值；只有 off 可以留空。',
