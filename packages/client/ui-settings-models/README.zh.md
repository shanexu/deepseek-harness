...
pi-ai profile 的 `models` 列表就在卡片上编辑：一行一个模型，行上显示 id 与显示名称，上下文窗口、输出上限与推理等级收在该行的展开区内，右侧是两个无文字的操作——展开与删除。空列表意味着「使用该路由的内置 catalog」，因此每一行都只会被刻意添加；清空容量会丢弃它，而不是存入一个 schema 会拒绝的值，配置留空的部分由适配器的路由级回退值定尺寸——留空的容量以这些回退值的量级作为占位符，那只是提示而非镜像：该字段按 1000 计 `K`，且部署可以覆盖这些回退值。不是正整数的容量根本不会被存下。推理等级编辑器写回该模型的 `reasoningEfforts`：继承／不支持思考（`false`）／按档位映射，映射值即发送给网关的线上拼写。它放在模型行而非路由级共享字段旁，因为档位是**每模型**的能力——composer 的模型选择器给每个模型提供的正是此行声明的档位——因此卡片会在 Apply 之前就拒绝适配器会拒绝的映射（空、只有 off、或启用了档位却没有线上拼写），而不是等到宿主拒绝。
...
- **卡片上可编辑的只有 API 密钥与精选折叠区字段**：手写编辑器用 schema 通用的字段覆盖面换来了设计稿上的布局（[Agent Note](../../../.agents/notes/implemented/architecture/2026-07-30-web-config-plane.zh.md)）。两个家族都公开 `baseURL` 与模型的 `id`/`name`/`contextWindow`/`maxTokens`；pi-ai 模型行还公开 `reasoningEfforts`，手工声明的 pi-ai 路由还公开 `displayName` 与 `api`。重试策略、超时、路由自身的 `reasoning` 与 `thinkingBudgets`、DeepSeek 模型说明及其他进阶字段仍留在 `settings.yaml` 中；编辑器未展示的现有模型字段会予以保留。不带这些约定字段的 profile schema 只渲染该提示，两套精选布局则以 `llm-deepseek`/`llm-pi-ai` 这两个 namespace 的名字为键。
